...
from keras.preprocessing.image import imag_to_array
from kera.preprocessing.image import load_img
...
ap.add_argument("-i", "--dataset", required = True, help = "path to the input images dataset")
ap.add_argument("-m", "--model", required = True, 
               help = "path to trained orientation model")
args = vars(ap.parse_args())

#load the label names(ie. angles) form the HDF5 dataset
db = h5py.File(args["db"])
labelNames = [int(angle) for in db["label_names"][:]]
db.close()
